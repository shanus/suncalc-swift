...
class MoonUtils {
	...
	class func getMoonCoords(d:Double) -> GeocentricCoordinates {
		...
		var l:Double  = L + Constants.RAD() * 6.289 * sin(M);    // longitude
		var b:Double  = Constants.RAD() * 5.128 * sin(F);        // latitude
		var dt:Double = 385001 - 20905 * cos(M);     // distance to the moon in km
